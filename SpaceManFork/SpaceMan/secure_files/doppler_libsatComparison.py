...
def calculate_derivative_npoints(values, h, npoints):
    """
    ...
    return np.dot(coefficients, values) / h
...
def checkComm(sat_id, lora_cfg, ed_pos, tx_time):
    ...
    #STARLINK-1008
    #1 44714U 19074B 24283.19429838 .00001293 00000+0 10573-3 0 9997
    #2 44714 53.0550 345.2582 0001561 77.8098 282.3066 15.06389323271075    

    #sat_id="STARLINK-1008"
    #ed_pos.latitude=
    #ed_pos.longitude=
    ...
    print(f"Satellite height = {height:.2f} meters")
    # doppler
    ...
    time_offsets = np.linspace(-2 * derivative_step, 2 * derivative_step,derivative_npoints)
    #print(time_offsets)
    #print()
    half = derivative_npoints // 2
    ...
    # calculate Doppler shift and rate for the time vector
    ...
    doppler_hz_vec = f_received - lora_cfg.fc # Doppler shift in Hz  
    tx_index = derivative_npoints // 2

    doppler_hz_tx = doppler_hz_vec[tx_index]
    doppler_hz_sec_tx=calculate_derivative_npoints(doppler_hz_vec, derivative_step, derivative_npoints)
    #doppler_hz_sec_tx= np.gradient(doppler_hz_vec) / np.gradient(t_relative_vec_s)
    
    #doppler_hz_sec_tx = np.gradient(doppler_hz_vec, derivative_step)[derivative_npoints // 2]
    # Evaluate Doppler shift limit against the LoRa configuration
    Ds_error=evaluates_static_doppler(doppler_hz_tx,lora_cfg.sf,lora_cfg.bw,lora_cfg.fc)
    if Ds_error:
        #print("Fail Doppler exceeds Doppler shift limit")
        ds_pass=False
        #return False, 0
    else:
        ds_pass=True
    ...
    # evaluate Doppler rate limit against the LoRa configuration
    n_first_fail = computeFirstFail(lora_cfg, doppler_hz_sec_tx)
    status = n_first_fail > pkt_symbols
    ...
    while funcGetSymbolsN(new_lora_cfg) > n_first_fail:
        if new_lora_cfg.payload_len > 0:
            new_lora_cfg.payload_len -= 1
        else:
            break

    max_payload_len = new_lora_cfg.payload_len

    t2 = ts.from_datetime(tx_datetime)
    
    height2 = wgs84.height_of(satellite.at(t2)).m
    #print(f"Satellite height at tx = {height:.2f} meters")
    #print(f"Satellite height at tx = {height2:.2f} meters")

    ds_value = doppler_hz_tx
    dr_value = doppler_hz_sec_tx
    ds_error = 0 if ds_pass else 1  
    dr_error = 0 if status else 1
    print(f"[Doppler evaluation] {t_relative_s} s, {ds_value:.2f} Hz, {dr_value:.2f} Hz/s, {pkt_symbols} sym, 1st sym error {n_first_fail}, ds_error {ds_error}, dr_error {dr_error}")
